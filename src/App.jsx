...
// UTILS DE FECHA: Solución definitiva al desfase de días
const formatDisplayDate = (dateString) => {
    if (!dateString) return 'SIN FECHA';
    // Para fecha de vencimiento (formato YYYY-MM-DD puro)
...
const formatTimestamp = (isoString) => {
    ...
    // Para fecha de asignado (formato ISO completo con hora)
...
const Input = ({ placeholder, value, onChange, type = 'text', Icon, isPassword = false, ...props }) => {
    ...
    return (
        <div className="mb-3 w-full text-left">
            <div className="flex items-center rounded-xl h-12 px-4 border border-gray-800 bg-[#1C1C1E] focus-within:border-[#3ABFBC] transition-all">
                {Icon && <Icon size={16} className="text-[#A9A9A9] mr-3" />}
                <input
                ...
                />
                {isPassword && (
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-[#A9A9A9] p-1 transition-colors hover:text-white">
...
const StudentInfoModal = ({ isVisible, onClose, student, onUpdate }) => {
    ...
    const [editData, setEditData] = useState({ nombre: '', email: '', dni: '' });
    ...
    useEffect(() => {
        if (student && isVisible) {
            setEditData({ 
                ...
                email: student.email || '', 
                dni: student.dni ? student.dni.toString() : '' 
            });
            setMsg({ text: '', type: '' });
        }
    }, [student, isVisible]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setMsg({ text: 'ALUMNO ACTUALIZADO CON ÉXITO', type: 'success' });
            setTimeout(() => { onUpdate(); onClose(); }, 1500);
        } catch (e) { 
            const errDetail = e.response?.data?.detail || 'ERROR AL ACTUALIZAR';
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-6 backdrop-blur-sm text-left text-center flex flex-col items-center justify-center">
            <div className="bg-[#1C1C1E] w-full max-md:max-h-[90vh] overflow-y-auto max-w-md rounded-3xl border border-gray-800 p-8 shadow-2xl relative">
                ...
                <div className="text-center mb-8"><h2 className="text-2xl font-black italic text-white uppercase tracking-tighter text-center">DATOS DEL ALUMNO</h2></div>
                ...
                <div className="space-y-1 text-left">
                    <label className="text-[12px] font-black text-[#A9A9A9] uppercase ml-2 mb-2 block tracking-widest text-left">Nombre Completo</label>
                    <Input placeholder="NOMBRE" Icon={User} value={editData.nombre} onChange={e => setEditData({...editData, nombre: e.target.value})} />
                    <label className="text-[12px] font-black text-[#A9A9A9] uppercase ml-2 mb-2 block tracking-widest text-left">Email</label>
                    <Input placeholder="EMAIL" Icon={Mail} value={editData.email} onChange={e => setEditData({...editData, email: e.target.value})} />
                    <label className="text-[12px] font-black text-[#A9A9A9] uppercase ml-2 mb-2 block tracking-widest text-left">Documento (DNI)</label>
                    <Input placeholder="DNI" Icon={CreditCard} value={editData.dni} onChange={e => setEditData({...editData, dni: e.target.value})} />
                </div>
                <button onClick={handleSave} disabled={loading} className="w-full bg-[#3ABFBC] h-14 rounded-2xl font-black text-black mt-6 uppercase italic shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 text-center">
...
const ResetPasswordModal = ({ isVisible, onClose, targetUser, mode = 'profile' }) => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const errorMsg = e.response?.data?.detail || "ERROR: CLAVE ACTUAL INCORRECTA";
            setMsg({ text: errorMsg.toString().toUpperCase(), type: "error" }); 
        } finally { setLoading(false); }
    };

    return (
        <div className="fixed inset-0 z-[210] bg-black/95 flex items-center justify-center p-6 backdrop-blur-sm text-left">
            <div className="bg-[#1C1C1E] w-full max-sm:w-[95%] max-w-sm rounded-3xl border border-gray-800 p-8 shadow-2xl">
                <div className="text-center mb-6">
                    <Key size={32} className="text-[#3ABFBC] mx-auto mb-2" />
                    <h2 className="text-lg font-black italic text-white uppercase tracking-tighter">SEGURIDAD</h2>
                    <p className="text-[10px] text-[#A9A9A9] font-black uppercase tracking-widest mt-1 italic text-center">
                        CAMBIAR CLAVE DE: <span className="text-white">{targetUser.nombre || "PERFIL"}</span>
                    </p>
                </div>
                ...
                <div className="space-y-1 text-left">
                    <label className="text-[10px] font-black text-gray-500 uppercase ml-2 mb-1 block tracking-widest text-left">Contraseña Actual</label>
                    <Input placeholder="TU CLAVE ACTUAL" Icon={Lock} value={oldPass} onChange={e => setOldPass(e.target.value)} isPassword />
                    
                    <label className="text-[10px] font-black text-gray-500 uppercase ml-2 mt-4 mb-1 block tracking-widest text-left">Nueva Contraseña</label>
                    <Input placeholder="NUEVA CLAVE" Icon={Zap} value={newPass} onChange={e => setNewPass(e.target.value)} isPassword />
                    <Input placeholder="CONFIRMAR NUEVA" Icon={CheckCircle} value={confirm} onChange={e => setConfirm(e.target.value)} isPassword />
                </div>
                
                <div className="flex gap-3 mt-8">
                    <button onClick={onClose} className="flex-1 bg-black border border-gray-800 h-12 rounded-xl text-white font-bold uppercase text-[10px] tracking-widest transition-all active:scale-95">CANCELAR</button>
                    <button onClick={handleUpdate} disabled={loading} className="flex-1 bg-[#3ABFBC] text-black h-12 rounded-xl font-black uppercase text-[10px] tracking-widest italic flex items-center justify-center transition-all active:scale-95 shadow-lg shadow-[#3ABFBC]/20">
    ...
    return (
        <div className="fixed inset-0 z-[250] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl text-center">
            <div className="bg-[#1C1C1E] w-full max-w-sm rounded-[2.5rem] border border-gray-800 p-10 shadow-2xl animate-in zoom-in duration-300">
                ...
                <h2 className="text-2xl font-black italic text-white uppercase tracking-tighter mb-4 leading-tight">¿RESET DE CLAVE?</h2>
                <p className="text-[12px] font-black text-gray-400 uppercase tracking-widest italic leading-relaxed mb-8">
                    ESTÁS POR RESETEAR LA CLAVE DE <span className="text-white">{student.nombre}</span> A LA CLAVE POR DEFECTO: <br/><span className="text-[#3ABFBC]">"tunuevacontraseña"</span>
                </p>
                <div className="flex flex-col gap-3">
                    <button onClick={onConfirm} disabled={loading} className="w-full bg-red-600 h-14 rounded-2xl font-black text-white italic uppercase shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all">
                        {loading ? <Loader2 className="animate-spin" /> : "SÍ, RESETEAR AHORA"}
                    </button>
                    <button onClick={onClose} disabled={loading} className="w-full bg-black border border-gray-800 h-14 rounded-2xl font-black text-gray-500 italic uppercase active:scale-95 transition-all text-center">
                        CANCELAR
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-black flex items-center justify-center p-8 relative overflow-hidden text-center">
            <div className="absolute inset-0 z-0">
                <img src="https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1500&auto=format&fit=crop" className="w-full h-full object-cover opacity-20 grayscale" />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-black" />
            </div>
            <div className="w-full max-sm:px-4 max-w-sm z-10 flex flex-col items-center">
                <img src={LOGO_URL} className="w-64 h-64 object-contain mb-6 filter drop-shadow-[0_0_30px_rgba(58,191,188,0.5)]" />
                <p className="text-[14px] font-black text-white uppercase tracking-[0.4em] mb-8 italic text-center">ES HORA DE LLEGAR MUY LEJOS</p>
                {err && <div className="bg-red-900/60 border border-red-500 text-white p-3 rounded-xl text-center mb-4 font-bold text-[10px] uppercase w-full tracking-widest">{err}</div>}
                ...
                <button onClick={handleLogin} disabled={load} className="w-full bg-[#3ABFBC] h-14 rounded-2xl font-black text-lg text-black mt-6 shadow-xl hover:scale-[1.02] active:scale-95 transition-all italic uppercase flex items-center justify-center text-center">
...
const ProfessorDashboard = ({ navigate }) => {
    ...
    const handleConfirmReset = async () => {
        ...
        try {
            await axios.patch(`${API_URL}/users/student/${student.id}`, {
                password: "tunuevacontraseña"
            }, { headers: { Authorization: `Bearer ${authToken}` } });
            setToast({ msg: `¡CLAVE DE ${student.nombre} RESETEADA!`, type: 'success' });
            setResetConfirm({ visible: false, student: null, loading: false });
        } catch (e) {
            setToast({ msg: 'ERROR AL RESETEAR LA CLAVE', type: 'error' });
    ...
    return (
        <div className="min-h-screen bg-black flex flex-col text-left">
            ...
            <ConfirmResetModal 
                isVisible={resetConfirm.visible} 
                student={resetConfirm.student} 
                loading={resetConfirm.loading}
                onConfirm={handleConfirmReset} 
                onClose={() => setResetConfirm({ visible: false, student: null, loading: false })} 
            />
            ...
            <header className="p-4 bg-[#1C1C1E] border-b border-gray-800 flex justify-between items-center sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                        <h2 className="text-[#3ABFBC] font-black text-xl italic uppercase tracking-tighter leading-none text-left">HOLA, {userData?.nombre?.split(' ')[0]}</h2>
                        <p className="text-[8px] font-black text-[#A9A9A9] uppercase tracking-widest mt-1 italic text-left">VAMOS POR TODO</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button onClick={refresh} className="w-10 h-10 bg-[#1C1C1E] border border-gray-800 rounded-xl flex items-center justify-center text-[#3ABFBC] hover:bg-[#3ABFBC]/10 transition-all shadow-sm"><RefreshCcw size={20}/></button>
                    <button onClick={() => setShowProfile(true)} className="w-10 h-10 bg-[#1C1C1E] border border-gray-800 rounded-xl flex items-center justify-center text-amber-500 hover:bg-amber-500/10 transition-all shadow-sm"><Key size={20}/></button>
                    <button onClick={() => navigate('addStudent')} className="w-10 h-10 bg-[#3ABFBC] rounded-xl flex items-center justify-center text-black shadow-lg hover:scale-105 transition-all"><UserPlus size={20}/></button>
                    <button onClick={signOut} className="w-10 h-10 bg-red-900/10 border border-red-500/30 rounded-xl flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm"><LogOut size={20}/></button>
                </div>
            </header>
            ...
            <main className="p-4 flex-1">
                <div className="relative mb-6 max-w-xl mx-auto">
                    ...
                    <input className="w-full bg-[#1C1C1E] h-12 pl-12 pr-4 rounded-xl text-white font-bold outline-none border border-gray-800 focus:border-[#3ABFBC] text-xs" placeholder="BUSCAR ALUMNO..." value={search} onChange={e => setSearch(e.target.value)}/>
                </div>

                {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-[#3ABFBC]" size={40}/></div> : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-20">
                        {filtered.map(s => (
                            <div key={s.id} className="bg-[#1C1C1E] rounded-3xl p-5 border border-gray-800 hover:border-[#3ABFBC]/40 transition-all shadow-2xl group">
                                <div className="flex items-center mb-6">
                                    <div className="w-14 h-14 rounded-2xl bg-[#3ABFBC] flex items-center justify-center mr-4 shadow-xl group-hover:scale-105 transition-transform"><User size={28} color="black" /></div>
                                    <div className="min-w-0 flex-1 text-left">
                                        <h3 className="text-sm font-black italic text-white uppercase truncate text-left">{s.nombre}</h3>
                                        <div className="flex flex-col gap-1 mt-1 text-left">
                                            <p className="text-[10px] font-black text-[#A9A9A9] uppercase tracking-tighter italic leading-none text-left">DNI: {s.dni}</p>
                                            <p className="text-[10px] font-black text-[#3ABFBC] lowercase truncate leading-none text-left tracking-tight italic">{s.email}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="grid grid-cols-4 gap-2">
                                    <button onClick={() => { setSelectedStudent(s); setShowInfo(true); }} className="flex flex-col items-center justify-center py-3 bg-black rounded-2xl border border-gray-800 text-[#3ABFBC] hover:bg-[#3ABFBC]/10 transition-all shadow-sm"><Info size={16}/><span className="text-[7px] font-black mt-1 uppercase text-center">Info</span></button>
                                    <button onClick={() => setResetConfirm({ visible: true, student: s, loading: false })} className="flex flex-col items-center justify-center py-3 bg-black rounded-2xl border border-gray-800 text-red-500 hover:bg-red-500/10 transition-all shadow-sm"><Key size={16}/><span className="text-[7px] font-black mt-1 uppercase text-center">RESET</span></button>
                                    <button onClick={() => navigate('viewRoutine', { studentId: s.id, studentName: s.nombre })} className="flex flex-col items-center justify-center py-3 bg-black rounded-2xl border border-gray-800 text-white hover:bg-white/10 transition-all shadow-sm"><History size={16}/><span className="text-[7px] font-black mt-1 uppercase tracking-widest text-center">Histo</span></button>
                                    <button onClick={() => navigate('createRoutineGroup', { studentId: s.id, studentName: s.nombre })} className="flex flex-col items-center justify-center py-3 bg-[#3ABFBC]/10 rounded-2xl border border-[#3ABFBC]/30 text-[#3ABFBC] hover:bg-[#3ABFBC] hover:text-black transition-all shadow-sm"><Dumbbell size={16}/><span className="text-[7px] font-black mt-1 uppercase tracking-widest italic text-center text-center">Nueva</span></button>
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        <div className="min-h-screen bg-black flex flex-col text-left">
            ...
            <header className="p-4 bg-[#1C1C1E] border-b border-gray-800 flex justify-between items-center sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                        <h2 className="text-[#3ABFBC] font-black text-xl italic uppercase tracking-tighter leading-none text-left">HOLA, {userData?.nombre?.split(' ')[0]}</h2>
                        <p className="text-[8px] font-black text-[#A9A9A9] uppercase tracking-widest mt-1 italic text-left">MI ENTRENAMIENTO</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setShowProfile(true)} className="w-10 h-10 bg-[#1C1C1E] border border-gray-800 rounded-xl flex items-center justify-center text-amber-500 hover:bg-amber-500/10 transition-all shadow-sm"><Key size={20}/></button>
            ...
            <main className="p-4 flex-1">
                <h3 className="text-white font-black italic uppercase tracking-tighter text-xl mb-6 border-l-4 border-[#3ABFBC] pl-3 text-left">MI PLAN ACTIVO</h3>
                    ...
                    <div className="space-y-4 pb-20">
                        {assignments.length === 0 ? (
                            <div className="text-center py-20 px-8 flex flex-col items-center">
                                ...
                                <p className="text-gray-500 font-black uppercase tracking-widest text-xs italic text-center">NO TENÉS RUTINAS ACTIVAS. <br/>CONTACTÁ A TU PROFESOR.</p>
                            </div>
                        ) : assignments.map(a => (
                            <div key={a.id} className="bg-[#1C1C1E] border border-gray-800 rounded-[2rem] overflow-hidden shadow-2xl">
                                <button onClick={() => setExpandedRoutine(expandedRoutine === a.id ? null : a.id)} className="w-full p-6 flex justify-between items-center bg-gradient-to-r from-black/0 to-[#3ABFBC]/5 transition-all active:bg-white/5">
                                    <div className="text-left">
                                        <p className="text-[#3ABFBC] font-black uppercase text-xl italic leading-none mb-1 text-left">{a.routine?.nombre}</p>
                                        <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest text-left">{a.routine?.routine_group?.nombre || "RUTINA"}</p>
                                    </div>
                                    <div className="bg-black w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-800 shadow-inner transition-transform" style={{ transform: expandedRoutine === a.id ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                                        <ChevronDown size={24} className={expandedRoutine === a.id ? "text-[#3ABFBC]" : "text-[#A9A9A9]"}/>
                                    </div>
                                </button>
                                
                                {expandedRoutine === a.id && (
                                    <div className="p-4 bg-black/40 space-y-4 animate-in slide-in-from-top-2 duration-300">
                                        {/* DESCRIPCIÓN DE LA RUTINA PARA EL ALUMNO */}
                                        {a.routine?.descripcion && (
                                            <div className="bg-[#3ABFBC]/5 border border-[#3ABFBC]/20 p-4 rounded-2xl flex items-start gap-3">
                                                <AlignLeft size={16} className="text-[#3ABFBC] mt-1 shrink-0" />
                                                <p className="text-[12px] text-gray-300 italic font-medium leading-relaxed uppercase">{a.routine.descripcion}</p>
                                            </div>
                                        )}
                                        {a.routine?.exercise_links?.map((link, i) => (
                                            <div key={i} className="bg-[#1C1C1E] border border-gray-800 p-5 rounded-2xl shadow-lg">
                                                <div className="flex flex-col gap-2 text-left">
                                                    <span className="text-lg text-white font-black italic uppercase tracking-tighter text-left">{link.exercise?.nombre}</span>
                                                    <div className="flex items-center gap-3 text-[#3ABFBC] font-black uppercase text-[12px] italic tracking-widest text-left">
                                                        <span>{link.sets} SERIES</span>
                                                        <span className="w-1 h-1 bg-gray-700 rounded-full"></span>
                                                        <span>{link.repetitions} REPS</span>
                                                        {link.peso && link.peso !== "0" && (
                                                            <>
                                                                <span className="w-1 h-1 bg-gray-700 rounded-full"></span>
                                                                <span className="text-amber-500">{link.peso} KG</span>
                                                            </>
                                                        )}
                                                    </div>
                                                    {link.notas && (
                                                        <div className="mt-2 bg-black/20 p-2 border-l-2 border-[#3ABFBC] text-left">
                                                            <p className="text-[10px] text-gray-400 italic text-left">{link.notas}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
...
const StudentRoutineView = ({ navigate, studentId, studentName }) => {
    ...
    const groupedAssignments = useMemo(() => {
        ...
        assignments.forEach(a => {
            ...
            const groupProf = a.professor?.nombre || "N/A";
            if (!groupsMap.has(key)) {
                groupsMap.set(key, { id: key, name: groupName, due_date: groupDue, professor_name: groupProf, is_active: false, date: a.assigned_at, items: [] });
            }
            const group = groupsMap.get(key);
            group.items.push(a);
            if (a.is_active) group.is_active = true;
        });
    ...
    const handleToggleGroupActive = async (group) => {
        ...
        try {
            ...
            const promises = group.items.map(a => 
                axios.patch(`${API_URL}/assignments/${a.id}`, { is_active: nextStatus }, { headers: { Authorization: `Bearer ${authToken}` } })
            );
    ...
    return (
        <div className="min-h-screen bg-black flex flex-col p-4 text-left">
            <header className="mb-6 text-left">
                <button onClick={() => navigate('dashboard')} className="text-[#3ABFBC] flex items-center gap-2 font-black italic uppercase tracking-tighter mb-4 text-sm hover:underline text-left"><ArrowLeft size={18}/> VOLVER</button>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pb-10 custom-scrollbar text-left">
                    {groupedAssignments.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-32 px-8 text-center flex flex-col items-center justify-center">
                           ...
                           <button onClick={() => navigate('createRoutineGroup', { studentId, studentName })} className="mt-10 px-8 h-14 bg-white/5 hover:bg-[#3ABFBC] text-[#3ABFBC] hover:text-black border border-[#3ABFBC]/20 rounded-2xl font-black uppercase tracking-widest italic transition-all flex items-center gap-3 active:scale-95 group shadow-xl flex items-center justify-center">
                               <PlusCircle size={20} className="group-hover:rotate-90 transition-transform text-center" />
                               NUEVO PLAN
                           </button>
                        </div>
                        ...
                        <div key={group.id} className={`rounded-[2rem] border bg-[#1C1C1E] transition-all overflow-hidden ${group.is_active ? 'border-[#3ABFBC] shadow-[0_0_20px_rgba(58,191,188,0.1)]' : 'border-gray-800'}`}>
                            <div onClick={() => setExpandedGroup(expandedGroup === group.id ? null : group.id)} className="p-6 flex justify-between items-center cursor-pointer active:bg-white/5 transition-colors">
                                ...
                                <div className="flex flex-col gap-3 items-end">
                                    <button onClick={(e) => { e.stopPropagation(); handleToggleGroupActive(group); }} disabled={updating} className={`px-6 py-3 rounded-xl text-[12px] font-black uppercase italic shadow-md transition-all active:scale-95 flex items-center gap-2 ${group.is_active ? 'bg-red-600 text-white' : 'bg-[#3ABFBC] text-black'} text-center flex justify-center`}>{updating ? <Loader2 className="animate-spin" size={14}/> : group.is_active ? <><XIcon size={14}/> INACTIVAR</> : <><CheckCircle size={14}/> ACTIVAR</>}</button>
                                    <div className="bg-black/40 w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-800/50 shadow-inner">{expandedGroup === group.id ? <ChevronUp size={28} className="text-[#3ABFBC]"/> : <ChevronDown size={28} className="text-[#A9A9A9]"/>}</div>
                                </div>
                            </div>
                            {expandedGroup === group.id && (
                                <div className="bg-black/20 border-t border-gray-800/50 p-4 space-y-3 animate-in slide-in-from-top-2 duration-300">
                                    {group.items.map(a => (
                                        <div key={a.id} className="bg-[#1C1C1E] border border-gray-800 rounded-3xl overflow-hidden shadow-sm">
                                            ...
                                            {expandedRoutine === a.id && (
                                                <div className="p-4 bg-black/40 space-y-4 border-t border-gray-800/50 animate-in fade-in duration-300">
                                                    {a.routine?.descripcion && (
                                                        <div className="bg-black p-4 rounded-2xl border border-gray-800 text-left">
                                                            <p className="text-[10px] text-[#3ABFBC] font-black uppercase tracking-widest mb-1 italic">Descripción de la Rutina:</p>
                                                            <p className="text-[12px] text-gray-400 font-medium leading-relaxed uppercase italic">{a.routine.descripcion}</p>
                                                        </div>
                                                    )}
                                                    {a.routine?.exercise_links?.map((link, i) => (
                                                        <div key={i} className="bg-[#1C1C1E] border border-gray-800 p-5 rounded-[1.5rem] hover:border-[#3ABFBC]/30 transition-all text-left">
                                                            <div className="flex flex-col gap-3 text-left">
                                                                ...
                                                                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-white font-black italic uppercase tracking-widest text-[16px] text-left">
                                                                    <span className="text-[#3ABFBC]">{link.sets}</span> SERIES DE <span className="text-[#3ABFBC]">{link.repetitions}</span> REPETICIONES
                                                                    {link.peso && link.peso !== "0" && <><span className="mx-1 text-gray-800">|</span><span className="text-amber-500">{link.peso} KG</span></>}
                                                                </div>
...
const AddStudentPage = ({ navigate }) => {
    ...
    return (
        <div className="min-h-screen bg-black p-6 flex flex-col items-center text-left">
            <header className="mb-8 max-w-lg w-full text-left text-left">
                <button onClick={() => navigate('dashboard')} className="text-[#3ABFBC] flex items-center gap-2 font-black italic uppercase tracking-tighter mb-8 text-xs hover:underline text-left text-left"><ArrowLeft size={16}/> VOLVER</button>
            ...
            <form onSubmit={handleSubmit} className="max-w-lg w-full space-y-4 text-left">
                ...
                <button type="submit" disabled={loading} className="w-full bg-[#3ABFBC] h-16 rounded-[1.5rem] font-black text-black text-lg italic uppercase shadow-xl mt-6 transition-all active:scale-95 text-center flex justify-center items-center">
                    {loading ? <Loader2 className="animate-spin" /> : "REGISTRAR ALUMNO"}
                </button>
            </form>
        </div>
    );
...
const RoutineGroupPage = ({ navigate, studentId, studentName }) => {
    ...
    const updateDaysCount = (newCount) => {
        ...
        if (newCount > next.length) {
            for(let i=next.length; i<newCount; i++) next.push({ nombre: `DIA ${i+1}`, descripcion: '', exercises: [] });
    ...
    const handleFinalSubmit = async () => {
        ...
        const payload = {
            ...
            fecha_vencimiento: groupData.due_date, 
            days: groupData.days,
        ...
        try {
            await axios.post(`${API_URL}/routines-group/create-transactional`, payload, { headers: { Authorization: `Bearer ${authToken}` } });
            navigate('dashboard');
        } catch (e) { 
            console.error("Error al asignar plan:", e.response?.data);
        }
    ...
    return (
        <div className="min-h-screen bg-black flex flex-col p-4 text-left">
            <ExerciseSelectorModal isVisible={isSelectorOpen} onClose={() => setIsSelectorOpen(false)} existingExercises={availableExercises} 
                onAddExercise={(ex) => {
                    const next = [...routines];
                    next[currentDayIdx].exercises.push({...ex, sets: 3, repetitions: "10", peso: "0", notas: '', order: next[currentDayIdx].exercises.length + 1});
                    setRoutines(next);
                }} 
            />
            <header className="mb-6 max-w-2xl mx-auto w-full text-left text-left">
                    ...
                    <div className="space-y-4 text-left">
                        ...
                        <div className="bg-[#1C1C1E] p-10 rounded-[2.5rem] border border-gray-800 text-center shadow-2xl mt-6">
                            ...
                            <div className="flex justify-center items-center gap-10">
                                <button onClick={() => updateDaysCount(Math.max(1, groupData.days - 1))} className="w-14 h-14 bg-black border border-gray-800 text-white rounded-2xl flex items-center justify-center disabled:opacity-20 active:scale-90 transition-all shadow-lg" disabled={groupData.days <= 1}><Minus/></button>
                                <span className="text-7xl font-black text-[#3ABFBC] italic tabular-nums drop-shadow-[0_0_15px_rgba(58,191,188,0.3)]">{groupData.days}</span>
                                <button onClick={() => updateDaysCount(Math.min(5, groupData.days + 1))} className="w-14 h-14 bg-[#3ABFBC] text-black rounded-2xl flex items-center justify-center disabled:opacity-20 active:scale-90 transition-all shadow-lg" disabled={groupData.days >= 5}><Plus/></button>
                            </div>
                        </div>
                        <button onClick={() => setStep(2)} className="w-full bg-[#3ABFBC] h-18 rounded-[2rem] font-black text-black text-xl italic uppercase shadow-2xl mt-10 transition-all active:scale-95 text-center flex justify-center items-center h-16">SIGUIENTE PASO</button>
                    </div>
                ) : (
                    <div className="space-y-8 text-left">
                        {routines.map((day, dIdx) => (
                            <div key={dIdx} className="bg-[#1C1C1E] p-6 rounded-[2rem] border-l-8 border-[#3ABFBC] shadow-2xl text-left">
                                ...
                                {/* NUEVO CAMPO: DESCRIPCIÓN DE LA RUTINA (DÍA) */}
                                <div className="mb-6">
                                    ...
                                    <div className="flex items-start gap-3 bg-black border border-gray-800 rounded-2xl p-4 focus-within:border-[#3ABFBC] transition-all">
                                        ...
                                        <textarea 
                                            ...
                                            onChange={e => {const n = [...routines]; n[dIdx].descripcion = e.target.value; setRoutines(n);}} 
                                            placeholder="EJ: ENFOQUE EN FUERZA EXPLOSIVA Y CONTROL EXCÉNTRICO..." 
                                            rows={2}
                                ...
                                <div className="space-y-4 text-left">
                                    {day.exercises.map((ex, eIdx) => (
                                        <div key={eIdx} className="bg-black/50 p-6 rounded-2xl border border-gray-800 relative shadow-inner text-left">
                                            ...
                                            <div className="grid grid-cols-3 gap-4 mb-5 text-left">
                                                ...
                                                <div className="text-left text-left"><label className="text-[8px] font-black text-[#A9A9A9] uppercase mb-1 block text-left text-left">Reps</label><input type="text" value={ex.repetitions} onChange={e => {const n = [...routines]; n[dIdx].exercises[eIdx].repetitions = e.target.value; setRoutines(n);}} className="w-full bg-[#1C1C1E] rounded-xl p-3 text-white text-center font-bold border border-gray-700 text-sm shadow-sm"/></div>
                                                <div className="text-left text-left"><label className="text-[8px] font-black text-[#A9A9A9] uppercase mb-1 block text-left text-left">Peso/Carga</label><input type="text" value={ex.peso} onChange={e => {const n = [...routines]; n[dIdx].exercises[eIdx].peso = e.target.value; setRoutines(n);}} className="w-full bg-[#1C1C1E] rounded-xl p-3 text-white text-center font-bold border border-gray-700 text-sm shadow-sm"/></div>
                                            </div>
                                    ...
                                    <button onClick={() => { setCurrentDayIdx(dIdx); setIsSelectorOpen(true); }} className="w-full border-2 border-dashed border-gray-800 h-16 rounded-[1.5rem] text-[#A9A9A9] font-black uppercase text-[10px] tracking-widest mt-4 flex items-center justify-center gap-3 transition-all hover:border-[#3ABFBC] hover:text-[#3ABFBC] hover:bg-[#3ABFBC]/5 active:scale-95 text-center"><PlusSquare size={20} className="text-center" /> AÑADIR EJERCICIO AL BLOQUE</button>
                                </div>
                            </div>
                        ))}
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black bg-opacity-95 flex items-center justify-center p-4 backdrop-blur-3xl text-left">
            <div className="bg-[#1C1C1E] w-full max-w-xl rounded-[2.5rem] border border-gray-800 p-8 flex flex-col h-[80vh] shadow-2xl animate-in slide-in-from-bottom-10 duration-500">
                <div className="flex justify-between items-center mb-8 text-left text-left text-left"><h2 className="text-3xl font-black italic text-[#3ABFBC] uppercase tracking-tighter text-left text-left text-left">BIBLIOTECA</h2><button onClick={onClose} className="text-gray-500 hover:text-white transition-colors"><X size={32}/></button></div>
                {isCreating ? (
                    <div className="space-y-5 text-left text-left text-left">
                        <div className="text-left text-left text-left text-left">
                           <label className="text-[10px] font-black text-gray-500 uppercase ml-2 mb-1 block tracking-widest text-left text-left text-left text-left">Nombre del Ejercicio</label>
                           <Input placeholder="NOMBRE DEL EJERCICIO" value={newEx.nombre} onChange={e => setNewEx({...newEx, nombre: e.target.value})} />
                        </div>
                        <div className="text-left text-left text-left text-left">
                           <label className="text-[10px] font-black text-gray-500 uppercase ml-2 mb-1 block tracking-widest text-left text-left text-left text-left">Grupo Muscular</label>
                    ...
                    <>
                        <div className="relative mb-6 text-left text-left text-left text-left"><Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500" size={20} /><input className="w-full bg-black border border-gray-800 h-14 pl-14 pr-6 rounded-2xl text-white text-sm font-bold outline-none focus:border-[#3ABFBC] transition-all" placeholder="BUSCAR EN LA LISTA..." value={search} onChange={e => setSearch(e.target.value)} /></div>
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar text-left text-left text-left text-left">{filtered.map(ex => (
                            <button key={ex.id} onClick={() => { onAddExercise(ex); onClose(); }} className="w-full p-5 bg-black rounded-2xl border border-gray-800 flex justify-between items-center hover:border-[#3ABFBC] group transition-all text-left text-left text-left text-left text-left text-left"><div className="text-left text-left text-left text-left text-left text-left text-left text-left"><span className="text-white font-black uppercase text-sm block group-hover:text-[#3ABFBC] transition-colors text-left text-left text-left text-left text-left text-left">{ex.nombre}</span><span className="text-[10px] text-[#A9A9A9] font-black uppercase tracking-widest text-left text-left text-left text-left text-left text-left">{ex.grupo_muscular}</span></div><PlusCircle size={24} className="text-[#3ABFBC] opacity-10 group-hover:opacity-100 transition-all shadow-sm text-center" /></button>
                        ))}</div>
                        <button onClick={() => setIsCreating(true)} className="w-full bg-[#3ABFBC]/5 border border-[#3ABFBC]/20 text-[#3ABFBC] h-16 rounded-[1.5rem] text-[10px] font-black uppercase mt-6 italic transition-all hover:bg-[#3ABFBC] hover:text-black flex items-center justify-center gap-3 text-center text-center text-center"><PlusSquare size={20} className="text-center" /> CREAR NUEVO EJERCICIO</button>
                    </>
                )}
            </div>
        </div>
...
const AppWrapper = () => (
    <AuthProvider>
        ...
        <style>{`
            ...
            body { font-family: 'Inter', sans-serif; background-color: black; margin: 0; color: white; -webkit-font-smoothing: antialiased; overflow-x: hidden; }
            input::placeholder { color: #A9A9A9; font-weight: 900; font-size: 8px; letter-spacing: 0.2em; opacity: 0.5; text-transform: uppercase; }
            textarea::placeholder { color: #A9A9A9; font-weight: 900; font-size: 8px; letter-spacing: 0.2em; opacity: 0.5; text-transform: uppercase; }
